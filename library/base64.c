...
unsigned char mbedtls_ct_base64url_enc_char(unsigned char value)
{
    ...
    digit |= mbedtls_ct_uchar_in_range_if(52, 61, value, '0' + value - 52);
    digit |= mbedtls_ct_uchar_in_range_if(62, 62, value, '-');
    digit |= mbedtls_ct_uchar_in_range_if(63, 63, value, '_');
    return digit;
}
...
int mbedtls_base64url_encode(unsigned char *dst, size_t dlen, size_t *olen,
                          const unsigned char *src, size_t slen, int padding)
{
    size_t i, n;
    int C1, C2, C3;
    unsigned char *p;
    ...
    for (i = 0, p = dst; i < n; i += 3) {
        ...
        *p++ = mbedtls_ct_base64url_enc_char((C1 >> 2) & 0x3F);
        *p++ = mbedtls_ct_base64url_enc_char((((C1 &  3) << 4) + (C2 >> 4))
                                          & 0x3F);
        *p++ = mbedtls_ct_base64url_enc_char((((C2 & 15) << 2) + (C3 >> 6))
                                          & 0x3F);
        *p++ = mbedtls_ct_base64url_enc_char(C3 & 0x3F);
    }
...
/*
 * Decode a base64url-formatted buffer
 */
                          ...
                          const unsigned char *src, size_t slen)
{
    ...
    for (i = n = 0; i < slen; i++) {
        ...
        if (src[i] == '=') {
            ...
            if (mbedtls_ct_base64url_dec_value(src[i]) < 0) {
                return MBEDTLS_ERR_BASE64_INVALID_CHARACTER;
            }
        }
        n++;
    ...
    for (x = 0, p = dst; i > 0; i--, src++) {
        ...
        if (*src == '=') {
            ...
            x |= mbedtls_ct_base64url_dec_value(*src);
        }
    ...
    if(accumulated_digits > 0) {
        x = x << (6*(4-accumulated_digits));
        *p++ = MBEDTLS_BYTE_2(x);
